...
PdfOperation PdfEngine::pdf_to_jpg(const std::string& input, const std::string& output) {
  ...
  cmd << "gs -q -dNOPAUSE -dBATCH -dSAFER -sDEVICE=jpeg -r150 -dJPEGQ=85 -sOutputFile=\"" << output << "\" \"" << input << "\" 2>/dev/null";
...
PdfOperation PdfEngine::compress(const std::string& input, const std::string& output) {
  std::ostringstream cmd;
  cmd << "gs -q -dNOPAUSE -dBATCH -dSAFER -sDEVICE=pdfwrite -dCompatibilityLevel=1.4 -dPDFSETTINGS=/ebook -dCompressFonts=true -r150x150 -sOutputFile=\"" << output << "\" \"" << input << "\" 2>/dev/null";
  int result = std::system(cmd.str().c_str());
